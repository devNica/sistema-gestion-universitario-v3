import { type RegisteredAccountApplicationModel, type ProfessorRegistrationApplicationModel } from '@auth/domain/ports/application/application-domain.model'
import { type ProfesorUserRegistrationPort } from '@auth/domain/ports/application/application-domain.port'
...
export default class ProfesorUserRegistrationUseCase
implements ControllerInputPort<RegisteredAccountApplicationModel | never> {
  constructor (
    private readonly service: ProfesorUserRegistrationPort,
    private readonly presenter: PresenterOutputPort<RegisteredAccountApplicationModel>
  ) {}

  async handleRequest (request: HttpRequestModel<ProfessorRegistrationApplicationModel>): Promise<HttpResponseModel<RegisteredAccountApplicationModel>> {
