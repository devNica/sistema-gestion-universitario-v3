import { type KnowledgeAreaRegistrationModel } from '@backoffice/domain/ports/application/application-domain.model'
import { type KnowledgeAreaRegistrationPort } from '@backoffice/domain/ports/application/application-domain.port'
...
export default class KnowledgeAreaRegistrationUseCase implements ControllerInputPort<EmptyResponseModel> {
  constructor (
    private readonly service: KnowledgeAreaRegistrationPort,
    private readonly presenter: PresenterOutputPort<EmptyResponseModel>
  ) {}

  async handleRequest (request: HttpRequestModel<KnowledgeAreaRegistrationModel>): Promise<HttpResponseModel<EmptyResponseModel>> {
